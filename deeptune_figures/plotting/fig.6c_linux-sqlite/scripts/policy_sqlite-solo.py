...
# -*- coding: utf-8 -*-
"""
SQLite Sampling Policy Visualization

This script generates performance comparison plots for SQLite tuning strategies:
...
WAYFINDER_TL_DATA_FOLDER = 'linux-sqlite-redis-tl'
DATA_OUTPUT = '../pdfs/'
RANDOM_DATA_FILE = '../../../measurements/linux-sqlite_13_only_OS_random_single_core.xlsx'
...
def simulate_random_baseline(data_file, n_samples, n_trials, time_samples):
    ...
    Returns:
        Tuple of (time_data, experiment_data, max_latency)
    """
    ...
    max_latency = throughput.max()
    experiment_data = []
    for _ in range(n_trials):
        trial_data = throughput[:n_samples].copy()
        trial_data[trial_data <= 0] = max_latency
        experiment_data.append(trial_data)
        np.random.shuffle(throughput)

    return time_random, np.vstack(experiment_data), max_latency


def load_experiment_data(data_folder, subfolder=None, n_samples=N_SAMPLES, max_latency=None):
    ...
    Args:
        ...
        max_latency: Maximum latency value for marking crashes
    ...
    for results_file in results_files:
        ...
        mask = throughput <= 0
        latency = 1.0 / throughput
        if max_latency is not None:
            latency[mask] = max_latency

        latency = latency[sorted_idx][:n_samples]
        experiment_data.append(latency)
...
def plot_performance_with_crashes(ax1, ax2, time_data, performance_data, max_latency, color, label, window_size):
    """Plot performance metrics and crash rates on dual axes.

    Args:
        ax1: Primary axis for latency
        ax2: Secondary axis for crash rate
        time_data: Time values
        performance_data: Performance data array
        max_latency: Maximum latency value indicating a crash
        color: Plot color
        label: Plot label
    ...
    # Plot crash rate
    crash_vector = performance_data >= max_latency
...
def setup_plot():
    ...
    ax1.set_ylabel('Latency ($\mu$s/op)')
...
def main():
    ...
    time_samples = [118.39, 45.49, 45.51, 45.52, 45.56, 78.10, 45.52, 44.21, 43.04, 42.98, 43.05, 43.07]

    # Generate random baseline
    time_random, random_data, max_latency = simulate_random_baseline(RANDOM_DATA_FILE, N_SAMPLES, N_TRIALS, time_samples)
    plot_performance_with_crashes(ax1, ax2, time_random, random_data, max_latency, 'r', 'Random', SMOOTHING_WINDOW)

    # Load and plot DeepTune (no TL)
    deeptune_data, deeptune_time = load_experiment_data(WAYFINDER_DATA_FOLDER, max_latency=max_latency)
    cumulative_time = np.cumsum(np.mean(deeptune_time, axis=0)) + time_random
    plot_performance_with_crashes(ax1, ax2, cumulative_time, deeptune_data, max_latency, 'b', 'DeepTune', SMOOTHING_WINDOW)

    # Load and plot DeepTune with TL
    deeptune_tl_data, deeptune_tl_time = load_experiment_data(WAYFINDER_DATA_FOLDER, WAYFINDER_TL_DATA_FOLDER, max_latency=max_latency)
    cumulative_time_tl = np.cumsum(np.mean(deeptune_tl_time, axis=0)) + time_random
    plot_performance_with_crashes(ax1, ax2, cumulative_time_tl, deeptune_tl_data, max_latency, 'g', 'DeepTune+TL', SMOOTHING_WINDOW)
    ...
    output_path = os.path.join(DATA_OUTPUT, 'new_linux-sqlite_sampling_policy_no-tf.pdf')
